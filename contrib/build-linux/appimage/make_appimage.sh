...
APPIMAGE="$DISTDIR/electrum-primeai-$VERSION-x86_64.AppImage"
...
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-primeai-pypi.txt"
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-primeai-git.txt"
...
cp "$PROJECT_ROOT/electrum-primeai.desktop" "$APPDIR/electrum-primeai.desktop"
