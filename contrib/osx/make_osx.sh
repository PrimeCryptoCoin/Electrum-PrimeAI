...
PACKAGE=Electrum-PrimeAI
GIT_REPO=https://github.com/PrimeAIProject/electrum-primeai
...
info "Installing dependencies sepcific to primeai..."
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-primeai-pypi.txt \
    || fail "Could not install dependencies from pypi"
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-primeai-git.txt \
...
info "Creating .DMG"
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-primeai-$VERSION.dmg || fail "Could not create .DMG"

DoCodeSignMaybe ".DMG" "dist/electrum-primeai-${VERSION}.dmg"
