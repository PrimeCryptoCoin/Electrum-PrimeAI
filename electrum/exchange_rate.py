...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        dicts = await self.get_json('api.coingecko.com',
                                    '/api/v3/coins/primeai/market_chart?vs_currency=%s&days=1' % ccy)
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/primeai/market_chart?vs_currency=%s&days=max' % ccy)
...
class TXBit(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        dicts = await self.get_json('api.txbit.io',
                            '/api/public/getticker?market=PrimeAI/%s' % ccy)
